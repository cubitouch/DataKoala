...
test('converts shared time ranges', () => { assert.deepEqual(grafanaRange({ kind: 'rolling', amount: 30, unit: 'minute' }), { from: 'now-30m', to: 'now' }); assert.deepEqual(grafanaRange({ kind: 'all' }), { from: 'now-5y', to: 'now' }); assert.deepEqual(grafanaRange({ kind: 'custom', startDate: '2026-01-01', startTime: '00:00', endDate: '2026-01-02', endTime: '00:00' }), { from: '1767225600000', to: '1767312000000' }) })
