...
test('cohort topology follows a link-only producer to consumer across traces', () => {
  const producer = summarizeTraceForCohort([
    span({ traceId: ids[0], spanId: 'producer', service: 'publisher', kind: 'PRODUCER', durationMs: 30 })
  ], { traceId: ids[0], durationMs: 40, status: 'ok' })
  const consumer = summarizeTraceForCohort([
    span({
      traceId: ids[1],
      spanId: 'consumer',
      service: 'worker',
      kind: 'CONSUMER',
      durationMs: 120,
      links: JSON.stringify([{ traceId: ids[0], spanId: 'producer' }])
    })
  ], { traceId: ids[1], durationMs: 140, status: 'ok' })

  assert.equal(consumer.edges.length, 0, 'parent-child topology alone must not invent the linked edge')
  const aggregate = aggregateTraceCohort([producer, consumer])
  const linked = aggregate.edges.find((edge) => edge.sourceLabel === 'publisher' && edge.targetLabel === 'worker')
  assert.ok(linked, 'the sampled cohort should resolve the producer span reference from the other trace')
  assert.equal(linked.kind, 'async')
  assert.equal(linked.callCount, 1)
  assert.equal(linked.p50Ms, 120)
  assert.deepEqual(linked.traceIds, [ids[1]], 'link latency and presence belong to the consumer trace')
})
