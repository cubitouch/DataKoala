...
export interface PrometheusPresetPayload { queryMode: QueryMode; sql: string; promqlBuilder: PromqlBuilderState; prometheusTimeRange: BuilderTimeRange; prometheusStep: QuerySession['prometheusStep'] }
...
export const prometheusPresetAdapter: ExplorationPresetAdapter<PrometheusPresetPayload> = {
  capture: (session) => clone({ queryMode: session.queryMode, sql: session.sql, promqlBuilder: session.promqlBuilder, prometheusTimeRange: session.prometheusTimeRange, prometheusStep: session.prometheusStep }),
  parse(value) {
    ...
    const queryMode = parseQueryMode(value.queryMode), promqlBuilder = parseBuilder(value.promqlBuilder), prometheusTimeRange = parseTimeRange(value.prometheusTimeRange)
    return queryMode && promqlBuilder && prometheusTimeRange ? clone({ queryMode, sql: value.sql, promqlBuilder, prometheusTimeRange, prometheusStep: value.prometheusStep }) : null
  },
