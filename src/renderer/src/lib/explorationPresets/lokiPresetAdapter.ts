...
export interface LokiPresetPayload { queryMode: QueryMode; sql: string; lokiBuilder: LokiBuilderState; lokiTimeRange: BuilderTimeRange; lokiResultLimit: number; lokiGroupBy: string[]; lokiResultView: QuerySession['lokiResultView']; sqlVisualization: VisualizationConfiguration }
...
export const lokiPresetAdapter: ExplorationPresetAdapter<LokiPresetPayload> = {
  capture: (s) => clone({ queryMode: s.queryMode, sql: s.sql, lokiBuilder: s.lokiBuilder, lokiTimeRange: s.lokiTimeRange, lokiResultLimit: s.lokiResultLimit, lokiGroupBy: s.lokiGroupBy, lokiResultView: s.lokiResultView, sqlVisualization: s.sqlVisualization }),
  parse(value) {
    ...
    const queryMode = parseQueryMode(value.queryMode), lokiBuilder = parseBuilder(value.lokiBuilder), lokiTimeRange = parseTimeRange(value.lokiTimeRange), sqlVisualization = parseVisualization(value.sqlVisualization)
    return queryMode && lokiBuilder && lokiTimeRange && sqlVisualization ? clone({ queryMode, sql: value.sql, lokiBuilder, lokiTimeRange, lokiResultLimit: value.lokiResultLimit as number, lokiGroupBy: value.lokiGroupBy, lokiResultView: value.lokiResultView, sqlVisualization }) : null
  },
