...
import { LokiTrendChart, lokiTrendPresentation } from './LokiTrendChart.tsx'
...
describe('Loki trend brushing', () => {
  ...
  it('uses shared grouped chart presentation and SQL stacking semantics', () => {
    const grouped = { ...result, columns: [...result.columns, { name: 'level', dataTypeID: 0, dataTypeName: 'text' }], rows: [
      { timestamp: '2026-01-01T00:00:00Z', value: 2, level: 'INFO' }, { timestamp: '2026-01-01T00:00:00Z', value: 1, level: 'ERROR' },
      { timestamp: '2026-01-01T00:01:00Z', value: 4, level: 'INFO' }
    ], rowCount: 3 }
    const bar = lokiTrendPresentation(grouped, 'bar').option as { legend: unknown; tooltip: unknown; xAxis: { type: string }; series: Array<{ name: string; stack?: string }> }
    expect(bar.series.map(({ name }) => name).sort()).toEqual(['level=ERROR', 'level=INFO'])
    expect(bar.series.every(({ stack }) => stack === 'total')).toBe(true)
    expect(bar.legend).toBeTruthy(); expect(bar.tooltip).toBeTruthy(); expect(bar.xAxis.type).toBe('time')
    const area = lokiTrendPresentation(grouped, 'area').option as { series: Array<{ stack?: string }> }
    expect(area.series.every(({ stack }) => stack === 'total')).toBe(true)
    const line = lokiTrendPresentation(grouped, 'line').option as { series: Array<{ stack?: string }> }
    expect(line.series.every(({ stack }) => stack === undefined)).toBe(true)
  })
