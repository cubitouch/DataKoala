...
import { TimeRangeField } from '@components/time-range/TimeRangeField'
...
import { QueryUtilityActions } from '@components/QueryUtilityActions'
  ...
  return <div className="editor-pane builder-pane" data-builder-panel="" onKeyDown={onKeyDown}>
    ...
    <BuilderForm className={styles.form} data-builder-form="">
      <BuilderRow className={`${styles.row} ${styles.contextRow}`} data-builder-control-row="context">
        ...
        <FormField data-builder-field=""><Combobox label="Table or view" value={selectedRelation ? relationIdentity(selectedRelation) : ''} options={relationOptions} onChange={chooseTable} placeholder="Select a table or view…" searchable disabled={!selectedSchema} emptyMessage="No matching tables or views" invalidationKey={selectedSchema} /></FormField>
        <FormField data-builder-field=""><Combobox label="Time column" value={builder.timeColumn ?? ''} options={timeColumnOptions} onChange={chooseTimeColumn} placeholder={temporalColumns.length ? 'Select a time column…' : 'No date/time columns'} searchable disabled={!builder.table || temporalColumns.length === 0} emptyMessage="No matching date/time columns" invalidationKey={relationInvalidationKey} /></FormField>
        {effectiveTimeRange && timeFilterColumn ? <FormField data-builder-field=""><TimeRangeField value={effectiveTimeRange} onChange={(timeRange) => setBuilder({ timeRange }, tabId)} error={rangeError} columnName={timeFilterColumn.name}/></FormField> : <FormField data-builder-field=""><span className={styles.fieldLabel}>Time range</span><div className={styles.unavailable} aria-disabled="true">{!builder.table ? 'Select a table' : temporalColumns.length ? 'Select a time column' : 'No date/time columns'}</div></FormField>}
      </BuilderRow>
      <BuilderRow className={styles.row} data-builder-control-row="dimensions">
        <FormField data-builder-field=""><Combobox label="X axis" value={selectedX ?? ''} options={xAxisOptions} onChange={chooseXAxis} placeholder="Select an X axis…" searchable disabled={!builder.table} emptyMessage="No matching columns" invalidationKey={relationInvalidationKey} /></FormField>
        <FormField data-builder-field=""><Combobox label="Y axis" hint="Optional for Count" value={selectedY ?? ''} options={yAxisOptions} onChange={chooseY} placeholder={aggregation === 'count' ? 'Count rows (no Y axis)' : 'Select a numeric Y axis…'} searchable disabled={!builder.table} emptyMessage="No matching numeric columns" invalidationKey={`${relationInvalidationKey}\0${selectedX ?? ''}\0${builder.seriesColumns.join('\0')}`} />{yRequired && !selectedY && <small className="inline-error" role="status">{aggregationLabel(aggregation)} requires a numeric Y axis column.</small>}</FormField>
        <FormField data-builder-field=""><MultiCombobox label="Series" values={builder.seriesColumns} options={seriesColumnOptions} onChange={(nextSeriesColumns) => void selectSeries(nextSeriesColumns)} placeholder="No breakdown" searchable showChips disabled={!builder.table || seriesProbe?.status === 'checking'} invalidationKey={`${relationInvalidationKey}\0${selectedX ?? ''}\0${selectedY ?? ''}`} />{seriesProbe?.status === 'checking' && <small role="status">Checking cardinality…</small>}{seriesProbe?.status === 'error' && <small className="inline-error" role="alert">{seriesProbe.message} <button className="btn ghost" onClick={seriesProbe.retry}>Retry</button></small>}</FormField>
      </BuilderRow>
      <BuilderRow className={`${styles.row} ${styles.transformationsRow}`} data-builder-control-row="transformations">
        {xTemporal ? <FormField data-builder-field=""><Combobox label="Time bucket" value={builder.timeBucket} options={timeBucketOptions} onChange={(value) => { probeGuard.current.invalidate(); setSeriesProbe(null); setBuilder({ timeBucket: value as TimeBucket }, tabId) }} /></FormField> : <div className={styles.slotEmpty} aria-hidden="true"/>}
        <FormField data-builder-field=""><Combobox label="Aggregation" value={aggregation} options={aggregationOptions} onChange={chooseAggregation} disabled={!builder.table} /></FormField>
      </BuilderRow>
    </BuilderForm>
