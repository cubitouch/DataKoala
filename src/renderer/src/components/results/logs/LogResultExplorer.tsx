...
export function LogResultExplorer({ rows, truncated, limit, selectionKey, onFilter }: Props) {
  ...
  const visible = useMemo(() => { const term = search.toLowerCase(); return term ? rows.filter((row) => effectiveLogMessage(row).toLowerCase().includes(term) || row.line.toLowerCase().includes(term) || JSON.stringify([row.labels, row.structuredMetadata, row.parsedFields]).toLowerCase().includes(term)) : rows }, [rows, search])
