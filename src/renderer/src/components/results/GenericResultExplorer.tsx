...
export function GenericResultExplorer({ mode, dimensionControls = 'result', hasRun = true, result, resultRevision, running, error, isResultStale, reconnecting = false, configuration, seriesVisibility, activeFilters, externalSeriesColumns = [], timeBucket, chartTimeDomain = null, hidePicker = false, onConfigurationChange, onSeriesVisibilityChange, onAddFilter, onRemoveFilter, onClearFilters, onToggleFilterExecution, canPromoteFilter, canDemoteFilter, onReconnect, onTemporalRangeSelected }: GenericResultExplorerProps) {
  ...
  const effectiveConfiguration = useMemo(
    () => result ? deriveEffectiveVisualization(result, configuration, dimensionControls === 'external' && mode === 'builder' ? 'builder' : 'result', externalSeriesColumns) : configuration,
  ...
  }, [result, effectiveConfiguration, configuration, dimensionControls, onConfigurationChange])
  ...
  const semanticConfiguration = JSON.stringify({ view: configuration.view, x: configuration.xColumn, y: configuration.valueColumn, series: configuration.seriesColumns?.length ? configuration.seriesColumns : configuration.seriesColumn ? [configuration.seriesColumn] : [], aggregation: configuration.aggregation, resultRevision })
  ...
  const onBrushEnd = (params: { areas?: Array<{ coordRange?: unknown[] }> }) => {
    ...
    else onAddFilter(createResultRangeFilter(effectiveConfiguration.xColumn, range.startInclusive, range.endExclusive))
    const instance = ref.current?.getEchartsInstance()
  ...
  const applyPointAction = (action: ChartFilterAction) => {
    ...
    if (action === 'includeSeries' || action === 'excludeSeries' || action === 'includeSeriesAndX') {
      ...
      for (const filter of chartSeriesResultFilters(context, include)) onAddFilter(filter)
    }
    if (action === 'includeX' || action === 'excludeX' || action === 'includeSeriesAndX') {
      ...
      if (context.timeBucket) {
        ...
        if (range) onAddFilter(createResultRangeFilter(context.xColumn, range.startInclusive, range.endExclusive, exclude))
      } else {
        const operator: 'equals' | 'notEquals' | 'isNull' | 'isNotNull' = context.xValue == null ? (exclude ? 'isNotNull' : 'isNull') : (exclude ? 'notEquals' : 'equals')
        onAddFilter(createResultFilter(context.xColumn, operator, context.xValue))
      }
    }
    dismissPointMenu()
  }
  ...
  if (!hasRun) return <div className={styles.explorer} data-result-explorer><div className={styles.pane}><div className={styles.empty} data-result-empty>{mode === 'builder' ? 'Select a table and X axis, then run the query to explore the grouped result.' : 'Run a query to view its results.'}</div></div></div>
  return <div className={styles.explorer} data-result-explorer>
    {result && isResultStale && <div className={styles.staleBanner} role="status">
      <span><strong>Disconnected</strong> — showing results from the last successful query.</span>
      <button className="btn ghost" onClick={() => void onReconnect?.()} disabled={reconnecting || !onReconnect}>
    ...
    {!showChart || !result ? <ResultsTable mode={mode} rawResult={result} filteredResult={filteredResult} activeFilters={activeFilters} resultRevision={resultRevision} running={running} error={error} onAddFilter={onAddFilter} onRemoveFilter={onRemoveFilter} onClearFilters={onClearFilters} onToggleFilterExecution={onToggleFilterExecution} canPromoteFilter={canPromoteFilter} canDemoteFilter={canDemoteFilter}/> : <div className={styles.chart} data-result-chart>
      ...
      <ResultFilterBar filters={activeFilters} onRemove={onRemoveFilter} onClear={onClearFilters} onToggleExecution={onToggleFilterExecution} canPromote={canPromoteFilter} canDemote={canDemoteFilter}/>
