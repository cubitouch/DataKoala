...
describe('PromQL Builder controls', () => {
  ...
  it('shows contextual percentile and Rate window controls and keeps Generated PromQL collapsed', async () => {
    ...
    expect(screen.getByText(/How much history each calculation/)).toBeTruthy()
    fireEvent.focus(help)
    expect(screen.getByRole('tooltip').textContent).toMatch(/How much history each calculation/)
  ...
  it('fails closed for an ambiguous histogram calculation until the user chooses a representation', async () => {
    ...
    const warning = screen.getByRole('button', { name: 'Calculation warning' })
    expect(warning.getAttribute('aria-describedby')).toBeTruthy()
    expect(screen.getByText(/Histogram calculations are not generated until you choose a representation/)).toBeTruthy()
    fireEvent.focus(warning)
    expect(screen.getByRole('tooltip').textContent).toMatch(/Histogram calculations are not generated/)
    expect(screen.getByRole('button', { name: 'Open in PromQL mode' }).hasAttribute('disabled')).toBe(true)
