...
export function LogResultExplorer({ rows, truncated, limit, toolbarLeading, selectionKey, onFilter, onCorrelate }: Props) {
  ...
  return <section className={styles.logs} aria-label="Log results">
    <div className={styles.resultToolbar}>{toolbarLeading}<input aria-label="Search loaded logs" placeholder="Search loaded logs…" value={search} onChange={(event) => setSearch(event.target.value)} /><span className={styles.loadedCount}>{visible.length} loaded{truncated ? ` · limited to ${limit} · more available` : ''}</span></div>
