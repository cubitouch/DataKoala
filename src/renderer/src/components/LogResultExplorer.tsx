...
export function LogResultExplorer({ rows, truncated, limit, selectionKey, onFilter }: Props) {
  ...
  return <section className={styles.logs} aria-label="Log results">
    <div className={styles.resultToolbar}><TextInput mode="inline" aria-label="Search loaded logs" placeholder="Search loaded logs…" value={search} onValueChange={setSearch} /><span className={styles.loadedCount}>{visible.length} loaded{truncated ? ` · limited to ${limit} · more available` : ''}</span></div>
