...
export function LogResultExplorer({ rows, truncated, limit, selectionKey, onFilter, onCorrelate }: Props) {
  ...
  return <section className={styles.logs} aria-label="Log results">
    ...
    <div className={styles.logSplit} data-inspector-open={Boolean(selected) || undefined}>
      ...
      {selected && <aside className={styles.logInspector} aria-label="Selected log details"><header><div><strong>Log event</strong><span>{new Date(selected.timestampMs).toISOString()}</span></div><button type="button" className="btn ghost" onClick={() => setSelectedId(null)} aria-label="Close log details">Close</button></header><div className={styles.inspectorBody}><div className={styles.inspectorSeverity}><strong data-severity={selected.severity.toUpperCase()}>{selected.severity.toUpperCase()}</strong></div><p className={styles.fullLine}>{eventMessage(selected)}</p><div className={styles.logActions}><button className="btn ghost" onClick={() => void navigator.clipboard.writeText(eventMessage(selected))}>Copy message</button><button className="btn ghost" onClick={() => void navigator.clipboard.writeText(selected.line)}>Copy raw log</button>{selected.traceId && onCorrelate && <button className="btn ghost" onClick={() => onCorrelate(selected.traceId!)}>Open trace</button>}</div>{(selected.traceId || selected.spanId) && <dl className={styles.traceIdentifiers}>{selected.traceId && <div><dt>Trace ID</dt><dd><code>{selected.traceId}</code><button aria-label="Copy Trace ID" onClick={() => void navigator.clipboard.writeText(selected.traceId!)}>Copy</button></dd></div>}{selected.spanId && <div><dt>Span ID</dt><dd><code>{selected.spanId}</code><button aria-label="Copy Span ID" onClick={() => void navigator.clipboard.writeText(selected.spanId!)}>Copy</button></dd></div>}</dl>}<section className={styles.detailSection}><h3>Indexed labels</h3>{fields(selected.labels, 'label')}</section><section className={styles.detailSection}><h3>Structured metadata</h3>{fields(selected.structuredMetadata, 'field')}</section><section className={styles.detailSection}><h3>Parsed fields</h3>{fields(selected.parsedFields, 'field')}</section></div></aside>}
    </div>
  </section>
}
