...
function ActionIcon({ kind }: { kind: 'copy' | 'include' | 'exclude' | 'raw' | 'open' }) {
  return <svg viewBox="0 0 16 16" aria-hidden="true">{kind === 'copy' ? <><rect x="5" y="5" width="8" height="8" rx="1"/><path d="M3 11H2V2h9v1"/></> : kind === 'include' ? <path d="M8 3v10M3 8h10"/> : kind === 'exclude' ? <path d="M3 8h10"/> : kind === 'raw' ? <><path d="M3 3h10v10H3zM5 6h6M5 8h6M5 10h4"/></> : <path d="M6 3h7v7M13 3 5 11M3 6v7h7"/>}</svg>
}
