...
function PostgresConnectionModal({ existing, onClose, onSaved, onBack, active = true }: FormProps & { existing: ConnectionProfile | null }) {
  ...
  return <div className={[styles.modalOverlay].join(' ')} onClick={onClose}><div className={[styles.modal].join(' ')} role="dialog" aria-modal="true" aria-labelledby="postgres-connection-title" onClick={(e) => e.stopPropagation()}>
    ...
    <div className={[styles.field].join(' ')}><label htmlFor="profile-name">Profile name</label><TextInput id="profile-name" {...inputProps('name')} value={draft.name} onValueChange={(text) => set({ name: text })} placeholder="e.g. staging analytics" /></div>
    <div className={[styles.row].join(' ')}><div className={[styles.field].join(' ')}><label htmlFor="connection-host">Host</label><TextInput id="connection-host" {...inputProps('host')} value={draft.host} onValueChange={(text) => set({ host: text })} /></div><div className={[styles.field].join(' ')}><label htmlFor="connection-port">Port</label><TextInput id="connection-port" {...inputProps('port')} inputMode="numeric" value={draft.port} onValueChange={(text) => set({ port: text })} /></div></div>
    <div className={[styles.row].join(' ')}><div className={[styles.field].join(' ')}><label htmlFor="connection-database">Database</label><TextInput id="connection-database" {...inputProps('database')} value={draft.database} onValueChange={(text) => set({ database: text })} /></div><div className={[styles.field].join(' ')}><label htmlFor="connection-user">User</label><TextInput id="connection-user" {...inputProps('user')} value={draft.user} onValueChange={(text) => set({ user: text })} /></div></div>
    <div className={[styles.field].join(' ')}><label htmlFor="connection-password">Password <span className={[styles.opt].join(' ')}>— leave empty for proxy / IAM / .pgpass auth</span></label><TextInput id="connection-password" type="password" value={draft.password} onValueChange={(text) => set({ password: text })} /></div>
...
function LocalFilesConnectionModal({ existing, onClose, onSaved, onBack }: FormProps & { existing: LocalFilesProfile | null }) {
  ...
  return <div className={[styles.modalOverlay].join(' ')} onClick={onClose}><div className={[styles.modal].join(' ')} role="dialog" aria-modal="true" aria-labelledby="local-files-connection-title" onClick={(event) => event.stopPropagation()}><ConnectionFormHeader kind="local-files" editing={!!existing} onBack={onBack} /><div className={[styles.field].join(' ')}><label htmlFor="local-profile-name">Connection name</label><TextInput id="local-profile-name" value={name} onValueChange={setName} /></div><div className={[styles.field].join(' ')}><label>Data files</label><button type="button" className={['btn', 'ghost'].join(' ')} onClick={() => void choose()}>Choose files…</button><div className={[styles.pasteHint].join(' ')}>CSV, TSV, Parquet, JSON, JSONL, and NDJSON. Each file is exposed as a read-only SQL view.</div></div>{files.map((file, index) => <div className={[styles.row, styles.fileConnectionRow].join(' ')} key={file.path}><div className={[styles.field].join(' ')}><label title={file.path}>{file.path.split(/[\\/]/).pop()}</label><TextInput aria-label={`Table alias for ${file.path}`} value={file.alias} onValueChange={(text) => setFiles((current) => current.map((item, i) => i === index ? { ...item, alias: text } : item))} /></div><button type="button" className={['btn', 'ghost'].join(' ')} aria-label={`Remove ${file.path}`} onClick={() => setFiles((current) => current.filter((_, i) => i !== index))}>Remove</button></div>)}{message && <div className={[styles.testMsg, message.ok ? styles.ok : styles.err].join(' ')} role={message.ok ? 'status' : 'alert'}>{message.text}</div>}<div className={[styles.actions].join(' ')}><button type="button" className={['btn', 'ghost'].join(' ')} onClick={() => void test()} disabled={busy}>Test</button><button type="button" className={['btn', 'ghost'].join(' ')} onClick={onClose}>Cancel</button><button type="button" className={['btn', 'primary'].join(' ')} onClick={() => void save()} disabled={busy}>{busy ? 'Working…' : 'Save'}</button></div></div></div>
}
