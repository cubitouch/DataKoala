...
function TempoConnectionModal({ existing, onClose, onSaved, onBack }: FormProps & { existing: TempoProfile | null }) {
  ...
  const [datasources, setDatasources] = useState<TempoDatasourceOption[]>([]); const [datasourceUid, setDatasourceUid] = useState(existing?.transport.datasourceUid ?? ''); const [loading, setLoading] = useState(true)
  const [grafanaUrl, setGrafanaUrl] = useState(existing?.grafana?.baseUrl ?? ''); const [orgId, setOrgId] = useState(String(existing?.grafana?.orgId ?? 1))
  useEffect(() => { let active = true; setLoading(true); api.connections.tempo.discoverDatasources({ kind: 'gcx', ...(context.trim() ? { context: context.trim() } : {}) }).then((found) => { if (!active) return; setDatasources(found); setDatasourceUid((current) => current || (found.length === 1 ? found[0].uid : '')) }).catch((error) => { if (active) setMessage({ ok: false, text: failureMessage(error) }) }).finally(() => { if (active) setLoading(false) }); return () => { active = false } }, [context])
  const selected = datasources.find((item) => item.uid === datasourceUid)
  ...
  return <div className={styles.modalOverlay} onClick={onClose}><div className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="tempo-connection-title" onClick={(event) => event.stopPropagation()}><ConnectionFormHeader kind="tempo" editing={!!existing} onBack={onBack} /><div className={styles.field}><TextInput label="Connection name" id="tempo-name" value={name} onValueChange={setName} /></div><div className={styles.field}><TextInput label="gcx context" id="tempo-context" value={context} onValueChange={setContext} placeholder="Current gcx context" /></div><div className={styles.field}><Combobox label="Tempo datasource" value={datasourceUid} onChange={setDatasourceUid} loading={loading} disabled={loading} placeholder="Select a datasource" options={datasources.map((item) => ({ value: item.uid, label: item.name }))} /><div className={styles.pasteHint}>Optional for DataKoala queries; required for Grafana handoff.</div></div><CollapsibleSection title="Grafana handoff (optional)"><div className={styles.advancedFields}><TextInput label="Grafana base URL" value={grafanaUrl} onValueChange={setGrafanaUrl} placeholder="https://example.com/grafana" hint="Navigation only; queries still run through gcx." /><TextInput label="Grafana org ID" inputMode="numeric" value={orgId} onValueChange={setOrgId} /></div></CollapsibleSection>{message && <div className={[styles.testMsg, message.ok ? styles.ok : styles.err].join(' ')} role={message.ok ? 'status' : 'alert'}>{message.text}</div>}<div className={styles.actions}><button type="button" className="btn ghost" onClick={() => void test()} disabled={busy}>{busy ? 'Testing…' : 'Test trace access'}</button><button type="button" className="btn ghost" onClick={onClose}>Cancel</button><button type="button" className="btn primary" onClick={() => void save()} disabled={busy}>{busy ? 'Working…' : 'Save'}</button></div></div></div>
}
