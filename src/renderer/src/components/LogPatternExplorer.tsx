      ...
      <div className={styles.spacer} style={{ height: virtualizer.getTotalSize() }}>
        {virtualizer.getVirtualItems().map((item) => {
          ...
          return <article ref={(element) => measureCard(cluster.id, element)} data-index={item.index} data-pattern-card className={styles.card} key={cluster.id} data-expanded={open || undefined} style={{ transform: 'translateY(' + item.start + 'px)' }}>
            ...
            <div className={styles.meta}><div className={styles.metaTimes}><span>First {new Date(cluster.firstTimestampMs).toLocaleString()}</span><span>Last {new Date(cluster.lastTimestampMs).toLocaleString()}</span></div><div className={styles.metaActions}>{Object.entries(cluster.severities).map(([severity, count]) => <strong key={severity} className={styles.severityBadge} data-severity={severity.toUpperCase()}>{severity.toUpperCase()} {count}</strong>)}<button type="button" className="btn primary" onClick={() => onViewLogs(cluster.template, cluster.memberIds)}>View logs</button></div></div>
