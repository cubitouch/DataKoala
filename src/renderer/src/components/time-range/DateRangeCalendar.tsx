...
export function DateRangeCalendar({ value, onChange }: { value: CustomTimeRangeValue; onChange: (value: CustomTimeRangeValue) => void }) {
  ...
  return <div className="date-range-calendar"><div className="calendar-head"><button type="button" className="icon-btn" aria-label="Previous month" onClick={() => moveMonth(-1)}>‹</button><strong>{monthName(p.year, p.month)}</strong><button type="button" className="icon-btn" aria-label="Next month" onClick={() => moveMonth(1)}>›</button></div>
  <div className="month-year-picker"><label>Month<select aria-label="Month" value={p.month} onChange={(e) => setMonth(`${p.year}-${String(Number(e.target.value)).padStart(2, '0')}`)}>{Array.from({ length: 12 }, (_, i) => <option key={i+1} value={i+1}>{new Intl.DateTimeFormat('en', { month: 'short', timeZone: 'UTC' }).format(new Date(Date.UTC(2020, i, 1)))}</option>)}</select></label><label>Year<input aria-label="Year" type="number" value={p.year} onChange={(e) => setMonth(`${e.target.value.padStart(4, '0')}-${String(p.month).padStart(2, '0')}`)}/></label></div>
  <div className="calendar-grid" role="grid" aria-label="Date range calendar">{DOW.map((d) => <span className="dow" key={d}>{d}</span>)}{days.map((day) => { const dp = parseDateOnly(day); const selected = Boolean(start && visualEndDate && day >= start && day <= visualEndDate) || day === start; const marker = day === start || day === visualEndDate; return <button key={day} ref={(element) => { if (element) dayRefs.current.set(day, element); else dayRefs.current.delete(day) }} type="button" role="gridcell" aria-label={day} aria-selected={selected} tabIndex={day === focusDate ? 0 : -1} className={`calendar-day${dp.month !== p.month ? ' outside' : ''}${selected ? ' in-range' : ''}${marker ? ' marker' : ''}${day === today ? ' today' : ''}`} onFocus={() => setFocusDate(day)} onKeyDown={(e) => key(e, day)} onClick={() => select(day)}>{dp.day}</button> })}</div></div>
}
