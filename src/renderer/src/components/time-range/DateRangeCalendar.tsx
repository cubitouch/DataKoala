...
export function DateRangeCalendar({ value, onChange }: { value: CustomTimeRangeValue; onChange: (value: CustomTimeRangeValue) => void }) {
  ...
  <div className={styles.monthYearPicker}><Combobox label="Month" mode="inline" value={String(p.month)} onChange={(month) => setMonth(`${p.year}-${month.padStart(2, '0')}`)} options={Array.from({ length: 12 }, (_, i) => ({ value: String(i + 1), label: new Intl.DateTimeFormat('en', { month: 'short', timeZone: 'UTC' }).format(new Date(Date.UTC(2020, i, 1))) }))} /><TextInput label="Year" mode="inline" type="number" value={p.year} onValueChange={(year) => setMonth(`${year.padStart(4, '0')}-${String(p.month).padStart(2, '0')}`)}/></div>
