...
export function CustomTimeRangeField({ value, onChange, error }: { value: CustomTimeRangeValue; onChange: (value: CustomTimeRangeValue) => void; error?: string | null }) {
  ...
  return <div data-time-range-field=""><FieldChrome label="Custom range" controlKind="button" error={error}>
    {({ controlId, labelId, describedBy, invalid }) => <><span id={`${controlId}-value`} className={styles.srOnly}>{customRangeSummary(value)}</span><Popover open={isOpen} onOpenChange={(next, reason) => { if (next) open(); else if (reason !== 'toggle') cancel(); else setIsOpen(false) }} trigger={<PopoverSummaryTrigger>{customRangeSummary(value)}</PopoverSummaryTrigger>} ariaLabel={`Custom time range: ${customRangeSummary(value)}`} triggerButtonProps={{ id: controlId, 'aria-labelledby': `${labelId} ${controlId}-value`, 'aria-describedby': describedBy, 'aria-invalid': invalid }} popupType="dialog" contentRole="dialog" contentClassName={styles.content} preferredWidth={760} maxHeight={720} focusOptionsOnKeyboardOpen={false}><CustomTimeRangePopover draft={draft} setDraft={setDraft} onCancel={cancel} onConfirm={confirm}/></Popover></>}
  </FieldChrome></div>
}
