...
export function TimeRangeField({ value, onChange, error, columnName }: { value: BuilderTimeRange; onChange: (value: BuilderTimeRange) => void; error?: string | null; columnName?: string }) {
  ...
  return <div className="custom-range-field"><span className="builder-field-label">Time range{columnName && <span className="builder-field-context"> · {columnName}</span>}</span><Popover open={isOpen} onOpenChange={(next, reason) => { if (next) open(); else if (reason !== 'toggle') cancel(); else setIsOpen(false) }} trigger={<><span className={popoverStyles.summary}>{summary}</span><span className={popoverStyles.chevron} aria-hidden="true"/></>} ariaLabel={`${label}: ${summary}`} popupType="dialog" contentRole="dialog" contentClassName="custom-time-range-content" preferredWidth={760} maxHeight={680} focusOptionsOnKeyboardOpen={false}><TimeRangePopover draft={draft} setDraft={setDraft} onCancel={cancel} onConfirm={confirm}/></Popover>{error && <small className="inline-error" role="alert">{error}</small>}</div>
}
