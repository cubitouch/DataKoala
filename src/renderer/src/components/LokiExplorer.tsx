...
export function LokiExplorer({ connectionId }: { connectionId: string }) {
  ...
  return <main className={styles.workspace} aria-label="Loki explorer">
    ...
    <section className={styles.results} aria-label="Loki query results">{result?.resultKind === 'logs' ? <>
      ...
      {patternScope && <div className={styles.status} role="status">Showing {scopedLogRows.length} logs matching <code>{patternScope.template}</code>. <button type="button" className="btn ghost" onClick={() => setPatternScope(null)}>Clear pattern</button></div>}
