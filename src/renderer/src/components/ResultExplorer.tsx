...
export function ResultExplorer({ mode, dimensionControls = 'result', hasRun = true, resultOverride, configurationOverride, onConfigurationChange, hidePicker = false, onTemporalRangeSelected }: ResultExplorerProps) {
  ...
  const semanticSeries = appliedChart ? (Array.isArray((appliedChart.option as { series?: unknown[] }).series) ? (appliedChart.option as { series: unknown[] }).series : [(appliedChart.option as { series?: unknown }).series]).filter(Boolean) : []
  const semanticItems = semanticSeries.reduce<number>((total, series) => total + (Array.isArray((series as { data?: unknown[] }).data) ? (series as { data: unknown[] }).data.length : 0), 0)
  ...
  return <div className={styles.explorer} data-result-explorer>
    ...
    {!showChart || !result ? <ResultsTable mode={mode} rawResult={result} filteredResult={filteredResult} activeFilters={activeFilters} resultRevision={resultRevision}/> : <div className={styles.chart} data-result-chart>
