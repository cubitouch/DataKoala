...
export function ResultExplorer({ mode, dimensionControls = 'result', hasRun = true, resultOverride, configurationOverride, onConfigurationChange, hidePicker = false, onTemporalRangeSelected }: ResultExplorerProps) {
  ...
  const onAddFilter = useCallback((filter: Parameters<typeof addResultFilter>[1]) => addResultFilter(mode, filter, tabId), [addResultFilter, mode, tabId])
  ...
  const canPromoteTableFilter = useMemo(() => mode === 'builder' ? (filter: Parameters<typeof isBuilderFilterPromotable>[0]) => isBuilderFilterPromotable(filter, builder) : undefined, [builder, mode])
  const canDemoteFilter = useMemo(() => mode === 'builder' ? (filter: Parameters<typeof resultFilterDemotion>[0]) => resultFilterDemotion(filter, result?.columns.map((column) => column.name) ?? []) : undefined, [mode, result])
  ...
  return <div className={styles.explorer} data-result-explorer>
    ...
    {!showChart || !result ? <ResultsTable mode={mode} rawResult={result} filteredResult={filteredResult} activeFilters={activeFilters} resultRevision={resultRevision} running={running} error={error} onAddFilter={onAddFilter} onRemoveFilter={onRemoveFilter} onClearFilters={onClearFilters} onToggleFilterExecution={onToggleFilterExecution} canPromoteFilter={canPromoteTableFilter} canDemoteFilter={canDemoteFilter}/> : <div className={styles.chart} data-result-chart>
      ...
      <ResultFilterBar filters={activeFilters} onRemove={onRemoveFilter} onClear={onClearFilters} onToggleExecution={onToggleFilterExecution} canPromote={mode === 'builder' ? (filter) => isBuilderFilterPromotable(filter, { ...builder, xColumn: configuration.xColumn }) : undefined} canDemote={canDemoteFilter}/>
