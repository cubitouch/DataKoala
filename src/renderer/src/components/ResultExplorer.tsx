...
export function ResultExplorer({ mode, hasRun = true }: { mode: QueryMode; hasRun?: boolean }) {
  ...
  return <div className={styles.explorer} data-result-explorer>
    ...
    {effectiveConfiguration.view !== 'table' && result && mode === 'sql' && <div className={styles.visualizationControls}>
      ...
      <div className={styles.visualizationControl}><span>Series <span>(optional, multiple)</span></span><MultiCombobox label="Series columns" values={sqlSeriesValues} options={seriesOptions} onChange={updateSqlSeries} placeholder="No breakdown" searchable showChips emptyMessage="No matching columns" /></div>
    </div>}
