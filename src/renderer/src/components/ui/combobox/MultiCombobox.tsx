...
interface Props { label: string; values: string[]; options: ComboboxOption[]; onChange: (values: string[]) => void; onOpen?: () => void; placeholder?: string; searchable?: boolean; showChips?: boolean; disabled?: boolean; loading?: boolean; error?: string | null; loadingMessage?: string; emptyMessage?: string; invalidationKey?: unknown; allowCustomValue?: boolean }
...
export function MultiCombobox({ label, values, options, onChange, onOpen, placeholder = 'Select options…', searchable = true, showChips = false, disabled = false, loading = false, error = null, loadingMessage = 'Loading…', emptyMessage = 'No matching options', invalidationKey, allowCustomValue = false }: Props) {
