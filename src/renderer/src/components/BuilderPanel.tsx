  ...
  return <div className="editor-pane builder-pane" data-builder-panel="" onKeyDown={onKeyDown}>
    ...
    <div className={styles.form} data-builder-form="">
      <div className={`${styles.row} ${styles.contextRow}`}>
        <div className={styles.control}><span className={styles.fieldLabel}>Schema</span><Combobox label="Schema" value={selectedSchema} options={schemaOptions} onChange={chooseSchema} placeholder="Select a schema…" searchable emptyMessage="No matching schemas" loading={metadataStatus === 'loading'} error={metadataStatus === 'error' ? (storeMetadataError ?? 'Could not load schemas') : null} disabled={!tabConnectionId || metadataStatus === 'loading'} /></div>
