...
const views: Array<{ view: ResultView; label: string; path: ReactNode }> = [
  ...
  { view: 'area', label: 'Area', path: <><path d="m3 17 5-6 4 3 8-9v11H3Z" className="fill"/><path d="m3 17 5-6 4 3 8-9"/></> },
...
export function ChartPicker({ value, onChange }: { value: ResultView; onChange: (view: ResultView) => void }) {
  return <div className="result-view-bar" role="toolbar" aria-label="Result view">
    <span>View</span>
    {views.map(({ view, label, path }) => <button key={view} type="button" className={value === view ? 'active' : ''} aria-label={label} title={label} aria-pressed={value === view} onClick={() => onChange(view)}>
      <svg viewBox="0 0 24 24" aria-hidden="true">{path}</svg><span className="view-label">{label}</span>
    </button>)}
  </div>
}
