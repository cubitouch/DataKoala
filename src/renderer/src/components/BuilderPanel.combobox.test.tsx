...
describe('BuilderPanel axis-first controls', () => {
  ...
  it('updates table/view selection and renders two-line metadata subtitles', () => {
    ...
    expect(option.querySelector('.combobox-option-label')?.textContent).toBe('orders')
    expect(option.querySelector('.combobox-option-subtitle')?.textContent).toBe('table · demo_shop')
  ...
  it('uses a searchable X axis combobox for temporal and non-temporal columns and removes X from Series', () => {
    ...
    expect(timestamp.querySelector('.combobox-option-subtitle')?.textContent).toBe('timestamp with time zone')
