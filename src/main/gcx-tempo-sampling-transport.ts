...
export class SamplingGcxTempoTransport implements TempoTransport {
  ...
  async search(expression: string, request?: TempoQueryRequest): Promise<QueryResult> {
    ...
    perf?.complete({ rowCount: result.rows.length, completedChunks: 1 })
    return result
  }
  ...
  attributeValues(attribute: string): Promise<string[]> { return this.exhaustive.attributeValues(attribute) }
}
