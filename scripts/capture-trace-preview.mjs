...
async function showServiceMap(win) {
  ...
  await waitFor(win, `document.querySelector('[data-trace-service-map] canvas') && document.body.innerText.includes('Top bottlenecks') && document.body.innerText.includes('service-03') && document.body.innerText.includes('slow traces')`, 'Tempo cohort service map and bottleneck candidates')
  await sleep(500)
}
...
async function showDenseServiceMap(win) {
  ...
  await waitFor(win, `(() => {
    ...
    return Boolean(map?.querySelector('canvas')) && text.includes('Top bottlenecks') && text.includes('Hover or select to reveal service labels') && text.includes('60')
  })()`, 'dense 60-service Tempo map')
  await sleep(700)
}
