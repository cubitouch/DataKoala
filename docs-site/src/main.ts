...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  ...
  <main>
    ...
    <section class="feature"><div><p class="eyebrow">LOG EXPLORATION</p><h2>Investigate production logs with Loki</h2><p>Build LogQL from metadata-driven filters, correlate an error spike in the log-volume trend, and expand individual events without losing the distinction between indexed labels, structured metadata, and parsed fields. Trace IDs provide a clear seam for deeper Tempo correlation.</p></div>${shot('loki-log-explorer', 'Loki log investigation for production checkout timeouts with Builder filters, generated LogQL, an error spike, virtualized log rows, and expanded event details')}</section>
