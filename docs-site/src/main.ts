...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  ...
  <main>
    ...
    <section class="feature"><div><p class="eyebrow">DATA SOURCES</p><h2>One workspace, varied sources</h2><p>Connect PostgreSQL, BigQuery, read-only SQLite, local CSV/TSV/Parquet/JSON files, Prometheus, and Tempo. Your data remains on your machine and source services.</p></div>${shot('docs-data-sources', 'DataKoala workspace populated with synthetic data-source profiles')}</section>
    ...
    <section class="feature"><div><p class="eyebrow">DISTRIBUTED TRACING</p><h2>Follow a request across services</h2><p>Search Grafana Tempo traces, compare duration and root-span status, then open a trace as a proportional waterfall. Filter noisy span kinds or delayed async branches while keeping timing and OpenTelemetry context close at hand.</p></div>${shot('tempo-waterfall', 'Tempo trace waterfall showing a checkout request across services with span timing, status, filters, and a selected failing payment span')}</section>
...
gcloud auth application-default login</code></pre><p>If you work with several Google Cloud projects, you can set the current CLI project explicitly:</p><pre><code>gcloud config set project YOUR_PROJECT_ID</code></pre><p>DataKoala uses credentials provided by the Google Cloud authentication flow and does not store Google Cloud credentials itself.</p></div><div class="optional-setup"><h3>Using Prometheus / Tempo / Grafana?</h3><p><code>gcx</code> is required for Tempo trace connections and is also used for Grafana and Grafana Cloud-backed Prometheus connections. On macOS, the Prometheus formula additionally provides <code>promtool</code>, which DataKoala can use for PromQL formatting.</p><pre><code>brew install prometheus
brew install grafana/grafana/gcx
promtool --version
gcx --version

gcx login my-stack --server https://&lt;your-stack&gt;.grafana.net</code></pre><p>Tempo needs <code>gcx</code>, but does not need <code>promtool</code>. If <code>promtool</code> is installed somewhere outside your <code>PATH</code>:</p><pre><code>DATAKOALA_PROMTOOL_PATH=/path/to/promtool pnpm dev</code></pre></div><p>Found a rough edge? <a href="${repo}/issues">Share feedback in GitHub Issues</a>.</p></section>
  </main>
